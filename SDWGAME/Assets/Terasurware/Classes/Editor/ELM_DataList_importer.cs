		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(5); p.탈락음소 = (cell == null ? "" : cell.StringCellValue);
					...
					cell = row.GetCell(11); p.정답음성 = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(12); p.오답음성1 = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(13); p.오답음성2 = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(14); p.오답음성3 = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(15); p.오답음성4 = (cell == null ? "" : cell.StringCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
